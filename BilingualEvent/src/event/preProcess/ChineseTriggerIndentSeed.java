...
public class ChineseTriggerIndentSeed implements TriggerIndent {
	...
	public void loadErrata() {
		knownTrigger = Common
				.readFile2Map6("chinese_trigger_known" + Util.part);
			...
			for (int i = 0; i < pr.words.size(); i++) {
				...
				if (errata1.containsKey(word)) {
					...
					for (String trig : trigs) {
						EventMention em = new EventMention();
						em.setAnchorStart(document.content.indexOf(
								trig.charAt(0), position[0]));
						em.setAnchorEnd(document.content.lastIndexOf(
								trig.charAt(trig.length() - 1), position[1]));
						em.setAnchor(trig);
						triggerHash.add(em);
					}
				}
				...
				if (errata2.containsKey(word)) {
					for (ArrayList<String> ts : errata2.get(word)) {
						...
						for (int k = 0; k < ts.size() - 1; k++) {
							if (i + 1 + k < pr.words.size()
									&& ts.get(k)
											.equals(pr.words.get(i + 1 + k))) {
							} else {
								conflict = true;
								break;
							}
						...
						if (!conflict) {
							EventMention em = new EventMention();
							em.setAnchorStart(document.content.indexOf(
									trigger.charAt(0), position[0]));
							em.setAnchorEnd(document.content.lastIndexOf(
									trigger.charAt(trigger.length() - 1),
	...
	public ArrayList<EventMention> extractChen(String file,
			Collection<EventMention> existTriggers,
			HashSet<EventMention> knownTriggers) {
		ArrayList<EventMention> eventMentions = new ArrayList<EventMention>();
		ACEChiDoc document = new ACEChiDoc(file);
		ArrayList<SemanticRole> semanticRoles = new ArrayList<SemanticRole>(
				document.semanticRoles.values());
		...
		for (EventMention trigger : existTriggers) {
			...
			for (EventMentionArgument argument : trigger
					.getEventMentionArguments()) {
				EntityMention mention = argument.getEntityMention();
				if (mention.getType().equalsIgnoreCase("time")
						|| mention.getType().equalsIgnoreCase("value")) {
					continue;
				}
	...
	public ArrayList<EventMention> extractTrigger(String file) {
		...
		for (ParseResult pr : document.parseReults) {
			for (int i = 0; i < pr.words.size(); i++) {
				...
				int end = position[1];
				if (knownTrigger.containsKey(word)
						&& knownTrigger.get(word).contains(posTag)) {
					EventMention em = new EventMention();
					em.setAnchorStart(position[0]);
					em.setAnchorEnd(position[1]);
					em.setAnchor(word);
				...
				if (train_words.contains(word)
						&& !knownTrigger.containsKey(word)) {
					continue;
				}
				EventMention em = new EventMention();
				em.setAnchorStart(position[0]);
				...
				em.setAnchor(word);
				if (start == end) {
					if (BVs.containsKey(word) && BVs.get(word).contains(posTag)
							&& BVStruct.get(word).contains("BV")) {
						triggerHash.add(em);
					}
				} else if (start + 1 == end) {
					...
					if (BVs.containsKey(str1) && BVs.get(str1).contains(posTag)) {
						...
						} else if (pos2.containsKey(str2)
								&& pos2.get(str2).startsWith("V")) {
							pattern = "BV_verb";
						...
						if (BVStruct.get(str1).contains(pattern)) {
							...
							// if(!em.getAnchor().equals(em.inferFrom) &&
							// !allAnchors.contains(em.getAnchor()))
					...
					if (BVs.containsKey(str2) && BVs.get(str2).contains(posTag)) {
						if (pos2.containsKey(str1)
								&& pos2.get(str1).startsWith("V")) {
							pattern = "verb_BV";
		...
		// }
		// ArrayList<EventMention> chens = this.extractChen(file, lastMentions,
		// triggerHash);
	...
	public static void filterEMs(ACEChiDoc document,
	...
	public static void main(String args[]) {
		...
		for (EventMention em : ems) {
			...
			int position[] = ChineseUtil.findParseFilePosition(start, end,
	...
	public static void addEntry(HashMap<String, HashSet<String>> maps,
